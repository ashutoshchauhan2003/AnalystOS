    ...
    <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }}>
      <GlassPanel className="h-full bg-[linear-gradient(180deg,rgba(255,255,255,0.078),rgba(255,255,255,0.028))] p-5" glow="none">
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-[11px] uppercase tracking-[0.28em] text-cyan-200/[0.72]">{candidate.title}</p>
          ...
          <button
            ...
            className={[
              "rounded-full border px-3 py-2 text-xs uppercase tracking-[0.24em] transition duration-300",
              isShortlisted
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {candidate.badges.map((badge) => (
            <div key={badge} className="rounded-full border border-cyan-300/[0.16] bg-cyan-300/[0.08] px-3 py-2 text-xs uppercase tracking-[0.22em] text-cyan-100">
              {badge}
            </div>
          ))}
        </div>

        <div className="mt-6 rounded-[1.2rem] border border-white/[0.08] bg-slate-950/[0.72] p-4">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          {candidate.metrics.map((metric) => (
            <div key={metric.label} className="rounded-[1rem] border border-white/[0.08] bg-white/[0.03] px-4 py-3">
        ...
        <div className="mt-5">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {candidate.tools.map((tool) => (
              <div key={tool} className="rounded-full border border-cyan-300/[0.12] bg-cyan-300/[0.04] px-3 py-2 text-sm text-cyan-100/90">
                {tool}
              </div>
            ))}
          </div>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <GlowButton href={candidate.profileHref}>View Profile</GlowButton>
